...
#define  IMG_ERR_32_BIT_MEMORY_LIMITATION           (_IMG_ERR + 0x0147) // This 32-bit device is operating on a 64-bit OS with more than 3GB of physical memory.  This configuration could allocate 64-bit memory which is unsupported by the device.  To solve this problem, reduce the amount of physical memory in the system, or use imgCreateBuffer to allocate buffer space.
...
#define  IMG_ERR_IMAGE_NOT_32BYTE_ALIGNED           (_IMG_ERR + 0x0168) // Image widths must be 32-byte aligned to work correctly with Vision algorithms. Used when optimizing the PCIe latency.
#define  IMG_ERR_IMAGE_BORDER_NONZERO               (_IMG_ERR + 0x0169) // The image border is not zero. Used when optimizing the PCIe latency.
#define  IMG_ERR_POCL_FAULT                         (_IMG_ERR + 0x0170) // The Power Over Camera Link (PoCL) circuitry has encountered an overcurrent condition.  Please power down the computer to safely reset the PoCL circuitry.
#define  IMG_ERR_POCL_VIDEO_LOCK                    (_IMG_ERR + 0x0171) // The PoCL circuitry is providing power but can't get a video lock.
#define  IMG_ERR_POCL_BAD_FUSE                      (_IMG_ERR + 0x0172) // The PoCL circuitry cannot provide power due to a blown fuse.
#define  IMG_ERR_POCL_NO_AUX_POWER                  (_IMG_ERR + 0x0173) // The PoCL circuitry cannot provide power due to a missing auxilliary power connection.
#define  IMG_ERR_PULSE_UPDATE_NOT_SUPPORTED         (_IMG_ERR + 0x0174) // This IMAQ device does not support updating an existing pulse.
#define  IMG_ERR_LAST_ERROR                         (_IMG_ERR + 0x0174)
